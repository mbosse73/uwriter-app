...
**Inhaltsverzeichnis (LH-12):** `withTableOfContents(styledHtml, style)` vergibt `id="uw-toc-N"` an alle `h1`/`h2`/`h3` des fertigen Export-HTML und stellt eine Liste mit Sprungmarken voran. Bewusst als **Nachbearbeitung** und nicht als Zweig in `renderStyledHtml()`: dieselbe Funktion rendert auch die iA-Writer-Live-Vorschau, die nie ein Verzeichnis bekommen soll. Eingeschaltet über `exportState.toc` (`setExportToc()`), nur beim PDF-Format sichtbar; ohne Überschriften bleibt das HTML unverändert. Keine Seitenzahlen — die entstehen erst im Druckdialog des Browsers und sind aus dem Dokument heraus nicht ermittelbar (LH-14).
...
3. **Test-Suite vorhanden, aber mit klaren Grenzen:** seit 2026-08 liegt unter `test/` eine dauerhafte jsdom-Suite (`node test/run.js`, 65 Fälle). Sie deckt Logik, Renderer und DOM-Interaktion ab, aber **nicht** `window.print()`/PDF-Ausgabe, die File-System-Access-Pfade (Backup-Verzeichnis, Dateibibliothek), Layout oder Theme-Farben — jsdom rechnet davon nichts. Vollständige Liste der Lücken in `test/README.md`. Für diese Bereiche bleibt der Blick in den echten Browser die einzige Prüfung.
...
7. **Seitenzahlen/Kopfzeile fehlen im PDF-Export** (LH-14): Browser bieten keinen verlässlichen Weg, das aus dem Dokument heraus zu setzen. Das Inhaltsverzeichnis (LH-12) ist dagegen umgesetzt — siehe Abschnitt 5, `withTableOfContents()`.
